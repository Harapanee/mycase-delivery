...
usage: python3 tools/stage_assets.py [YYYY-MM-DD]
"""
import datetime
import json
import os
...
def main():
    start = datetime.datetime.strptime(sys.argv[1] if len(sys.argv) > 1 else "2026-09-13", "%Y-%m-%d")
    ...
    for i, k in enumerate(sorted(picks, key=int)):
        ...
        src = os.path.join(SRC, "renders", "batch", f"mycase-{n}-{t}.mp4")
        dst = os.path.join(OUT, key + ".mp4")
        if not os.path.exists(dst):
            shutil.copyfile(src, dst)
        subprocess.run(["ffmpeg", "-v", "error", "-y", "-ss", f"{fz + 1.5:.3f}", "-i", src,
